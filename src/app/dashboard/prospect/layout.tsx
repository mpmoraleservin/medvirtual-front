...
export default function ProspectDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    { href: "/dashboard/prospect", icon: Home, label: "Dashboard" },
    { href: "/dashboard/prospect/hire-requests", icon: ListChecks, label: "My Hire Requests" },
    { href: "/dashboard/prospect/talent-pool", icon: Users2, label: "Talent Pool" },
    { href: "/dashboard/prospect/profile", icon: User, label: "Profile" },
  ];
  return (
      ...
      <div className="flex-1 w-0 min-h-screen ml-0 sm:ml-64 flex flex-col">
        ...
        <nav className="sm:hidden fixed bottom-0 left-0 w-full h-16 bg-white border-t border-[#E9EAEB] flex justify-around items-center z-50 shadow-lg">
          {navItems.map(({ href, icon: Icon, label }) => {
            const isActive = pathname === href || (href !== "/dashboard/prospect" && pathname.startsWith(href));
            return (
