...
function getStatusColor(status: HiredStaffMember["status"]) {
  switch (status) {
...
function getStatusText(status: HiredStaffMember["status"]) {
  switch (status) {
  ...
  <Card className="p-8 border border-dashed border-gray-300 bg-gray-50">
    <div className="flex flex-col items-center justify-center text-center">
      ...
      <p className="text-gray-600 mb-4 max-w-md">
        Start building your team by submitting your first hire request. We        Start building your team by submitting your first hire request. WeWe'llapos;ll help you find the perfect candidates.apos;ll help you find the perfect candidates.
      </p>
      ...
      <div className="flex items-start gap-3 mb-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <Badge 
              variant="outline" 
              className={`text-xs px-2 py-0 ${getStatusColor(member.status)}`}
            >
...
const StatsOverview = () => {
  const activeStaff = hiredStaff.filter(s => s.status === "active").length;
  const totalBonuses = hiredStaff.reduce((sum, staff) => 
    sum + (staff.bonuses?.length || 0), 0
  );
  const totalMonthlyCost = hiredStaff
    .filter(s => s.status === "active")
    .reduce((sum, staff) => sum + staff.billRate, 0);
...
export default function ClientDashboardV2() {
  ...
  const displayedStaff = useMemo(() => {
    const staff = hiredStaff.filter(s => s.status !== "terminated");
    return showAllStaff ? staff : staff.slice(0, 6);
  }, [showAllStaff]);
      ...
      <main className="flex-1 p-6 md:p-10 w-full max-w-none px-4 sm:px-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-1">HereHere'sapos;s whatwhat'sapos;s happening with your team today</p>
          </div>
          ...
          <SheetContent side="right" className="w-[40vw] min-w-[400px] max-w-[48rem]">
            ...
            <div className="flex flex-col items-center gap-4 p-2">
              ...
              <div className="text-center">
                ...
                <Badge 
                  variant="outline" 
                  className={`mt-2 ${getStatusColor(selectedStaff?.status || "active")}`}
                >
                  {getStatusText(selectedStaff?.status || "active")}
                </Badge>
              </div>
